...
BlockHeader *expand_heap(size_t aligned_size) {
    ...
    kprintln("ALLOC: Expanding heap memory by %ld pages.", page_count);
    uint64_t allocation = pmm_alloc(page_count);
    if (allocation == 0) {
        kprintln("ALLOC: The system is out of memory to complete heap expansion.");
        return null;
    }

    BlockHeader *header = p2v(allocation);
